...
import { FcAlarmClock, FcApproval, FcCalendar, FcEmptyTrash, FcHighPriority } from 'react-icons/fc';
...
const Important = () => {
    ...
    const url = `https://mytask-server.vercel.app/myimportant?email=${user?.email}`;
    ...
    const taskRemove = id => {
        ...
        if (proceed) {
            fetch(`https://mytask-server.vercel.app/important/${id}`, {
                method: 'DELETE'
            })
                .then(res => res.json())
                .then(data => {
